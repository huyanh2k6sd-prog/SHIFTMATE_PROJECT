...
const getDayKey = (dateString) => {
    ...
    // In JS dates, 0 is Sunday. But we want Mon-Sun order.
    return days[d.getDay()];
};

// Helper to get date number (1-31)
const getDateNumber = (dateString) => {
    const d = new Date(dateString);
    return d.getDate();
};

export function ManagerAvailabilityGrid({ shifts = [], activeRole, weekDays = [], onOpenStaffModal, onOpenEditModal, onOpenDeleteModal }) {
    ...
    const handleAction = (action, shiftData) => {
        ...
        if (action === 'staff') onOpenStaffModal(shiftData);
        ...
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center h-64 border-2 border-dashed border-gray-200 dark:border-white/10 rounded-2xl w-full m-4">
                ...
                <p className="text-text-secondary-light dark:text-text-secondary-dark max-w-md mx-auto">
                    Please select a role from the sidebar or create a new role to manage shifts and availability.
                </p>
            </div>
        );
    }
                    ...
                    <div className="flex-1 p-3 flex flex-col gap-3">
                        {(shiftsByDate[day.fullDate] || []).map(shift => {
                            ...
                            const modalShiftData = {
                                ...
                                availableStaff: shift.available_count || 0
                            };
                            ...
                            return (
                                <div key={shift.id} className="relative">
                                    <div
                                        ...
                                        className="p-3 rounded-xl border shadow-sm hover:shadow-md transition-all cursor-pointer group min-h-[100px] flex flex-col justify-between w-full bg-white dark:bg-surface-dark border-gray-100 dark:border-white/10"
                                    >
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            ...
                                            <span className="text-[10px] font-bold text-text-primary-light dark:text-text-primary-dark">
                                                {shift.available_count || 0}/{shift.required_staff} available
                                            </span>
                                        </div>
                                    </div>
                                    {activeMenuId === shift.id && (
                                        <ShiftOptionsMenu
                                            ...
                                            onAvailableStaff={() => handleAction('staff', modalShiftData)}
